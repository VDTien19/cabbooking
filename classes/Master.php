...
Class Master extends DBConnection {
	...
	function save_cab(){
		...
		if(isset($price)){
			$check = $this->conn->query("SELECT * FROM `cab_list` where `price` = '{$price}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
			if($this->capture_err())
				return $this->capture_err();
			// if($check > 0){
			// 	$resp['status'] = 'failed';
		...
		if($save){
			...
			if(empty($id))
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					$fname = 'uploads/dirvers/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['img']['name'];
					$move = move_uploaded_file($_FILES['img']['tmp_name'],'../'.$fname);
					if($move){
						try {
							$sql = "UPDATE cab_list SET image_path = '$fname' WHERE id = $id";
				...
				if(isset($_FILES['car_image']) && $_FILES['car_image']['tmp_name'] != ''){
					$fname = 'uploads/cabs/'.strtotime(date('y-m-d H:i')).'_'.$_FILES['car_image']['name'];
					$move = move_uploaded_file($_FILES['car_image']['tmp_name'],'../'.$fname);
					if($move){
						try {
							$sql = "UPDATE cab_list SET image_car = '$fname' WHERE id = $id";
							$query = $this->conn->query($sql);
							if($query && $id == $this->settings->userdata('id') && $this->settings->userdata('login_type') == 2){
								$this->settings->set_userdata('image_car', $fname);
								// if ($id == $this->settings->userdata('id') && $this->settings->userdata('login_type') == 2) {
								// }
								echo "Upload and save image successfully.";
							} else {
