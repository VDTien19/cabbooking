...
<!-- Section-->
<section class="py-5">
    <div class="container px-4 px-lg-5 mt-5 card rounded-0 card-outline card-green shadow">
        <div class="row">
            <div class="col-md-12">
            <center>
            <h1 class="display-4 fw-bolder">Available Cabs</h1>
            <hr>
                ...
                <div class="row gx-4 gx-lg-5 row-cols-1 row-cols-sm-1 row-cols-md-2 row-cols-xl-3" id="cab_list">
                    ...
                    while($row= $cabs->fetch_assoc()):
                        $anhxe = ($row["image_car"] == "") ? "uploads/cabs/notcab.png" : $row["image_car"];
                    ?>
                    <a class="col item text-decoration-none text-dark book_cab" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>" data-bodyno="<?php echo $row['body_no'] ?>">
                        <div class="callout callout-primary border-success rounded-0">
                            <div style="width:100%;height:200px"><img style="width:100%;height:200px;object-fit:cover;border-radius:5px" src="<?=$anhxe?>" alt=""></div>
                            <dl>
                                <dt class="h3"><i class="fa fa-taxi"></i> <?php echo $row['body_no'] ?></dt>
                                <dd class="truncate-3 text-muted lh-1">
                                    <p><strong>Loại: </strong> <?php echo $row['category'] ?></p>
                                    <p><strong>Tên: </strong ><?php echo $row['cab_model'] ?></p>
                                    <p><strong>Giá: </strong ><?php echo $row['price'] ?>vnđ / km</p>
                                </dd>
                            </dl> 
                        </div>
                    </a>
